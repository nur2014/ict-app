<template>
  <div>
    <b-overlay :show="loadingState">
        <template v-if="dataFilter.officeId">
            <div>
                <div class="row justify-content-md-center">
                    ...
                    <div class="col-lg-8 text-center">
                        ...
                        <h4 class="org-name" style="font-size:19px;">{{ getOfficeName(dataFilter.officeId) }}</h4>
                        <h6 class="org-address">{{ getAddressName(dataFilter.officeId)}}</h6>
                    </div>
        ...
        <template v-else-if="dataFilter.orgId">
            <div>
                <div class="row justify-content-md-center">
                    <div class="col col-lg-2 text-right">
                        <b-img width='70px' :src="'data:image/jpeg;base64,' + leftLogo" fluid alt="Left Logo"></b-img>
...
export default {
  ...
  computed: {
    dataFilter () {
        return this.$store.state.dataFilters
    },
    langChange () {
  ...
  methods: {
    ...
    getOfficeName (offceId) {
        const officeNameObj = this.$store.state.commonObj.officeList.find(item => item.value === offceId)
        return typeof officeNameObj !== 'undefined' ? (this.langChange === 'bn' ? officeNameObj.text_bn : officeNameObj.text_en) : ''
    },
    getAddressName (offceId) {
        const officeNameObj = this.$store.state.commonObj.officeList.find(item => item.value === offceId)
        return typeof officeNameObj !== 'undefined' ? (this.langChange === 'bn' ? officeNameObj.address_bn : officeNameObj.address) : ''
    }
  }
}
</script>
