        ...
        <div>
          <div class="row justify-content-md-center">
            ...
            <div class="col-md-auto text-center">
              <!-- <h3 class="text-bold">{{ $t('globalTrans.government_republic_of_bangladesh') }}</h3> -->
    ...
    getReportHead (orgId) {
      ...
      RestApi.getData(this.baseUrl, this.uri + '/' + this.orgId, this.getParams).then(response => {
        if (typeof response.success !== 'undefined' && response.success) {
          const orgData = this.$store.state.Portal.agriObj.organizationProfileList.find(item => item.value === this.orgId)
